...
class Guest
  ...
  attr_reader :name, :number_of_friends, :time_of_arrival, :social

  def initialize(name, number_of_friends, time_of_arrival, social)
    @name = name
  ...
  def add_to_list
    CSV.open(CSV_FILE_NAME, 'a') do |csv|
      csv << [name, number_of_friends, time_of_arrival, social]
    end
  end
